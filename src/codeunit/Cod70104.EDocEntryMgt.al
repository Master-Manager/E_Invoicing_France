codeunit 70104 "EDoc Entry Mgt."
{
    ...
    /// <summary>
    /// Richer creation entry point that also links the EDoc Entry back to its BC source document
    /// (mirroring the real E-Document table's "Document Record ID"/"Table ID"). Use this from the
    /// posting subscribers once document preparation (mapping) is wired in.
    /// </summary>
    procedure CreateEntryFromDocument(DocumentType: Enum "EDoc Document Type"; FlowType: Enum "EDoc Flow Type"; EDocDirection: Enum "EDoc Direction"; SourceType: Enum "EDoc Source Type"; SourceNo: Code[20]; var SourceDocumentHeader: RecordRef): Integer
    var
        Entry: Record "EDoc Entry";
        SetupMgt: Codeunit "EDoc Setup Mgt.";
        Service: Record "EDoc Service";
    begin
        ...
        Entry.Init();
        Entry."Document Type" := DocumentType;
        Entry."Flow Type" := FlowType;
        Entry.Direction := EDocDirection;
        Entry."Source Type" := SourceType;
        Entry."Source No." := SourceNo;
        Entry.SetSourceDocument(SourceDocumentHeader);
        Entry."Service Code" := Service.Code;
